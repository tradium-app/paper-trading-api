...
module.exports = async function () {
	try {
		...
		let todayOutcomes = await axios.get(
			`http://portal.edcd.gov.np/rest/api/fetch?filter=outcomeOfDay&type=dayByDay&eDate=${today}&disease=COVID-19`,
		)
		if (todayOutcomes.data.length === 0) {
			todayOutcomes = await axios.get(
		...
		if (todayCasesbyDistricts.data.length === 0) {
			todayCasesbyDistricts = await axios.get(
		...
		allOutcomes.data &&
			allOutcomes.data.forEach((district) => {
				totalDeaths += parseInt(district['Number of deaths'])
				totalRecovered += parseInt(district['Number of cases recovered'])
			})

		let allCases = await axios.get(
			`https://portal.edcd.gov.np/rest/api/fetch?filter=casesBetween&type=dayByDay&eDate=${today}&disease=COVID-19`,
			{ httpsAgent },
		)
		if (allCases.data.length === 0) {
			allCases = await axios.get(
				`https://portal.edcd.gov.np/rest/api/fetch?filter=casesBetween&type=dayByDay&eDate=${yesterday}&disease=COVID-19`,
				{ httpsAgent },
			)
		}
